...
void lose_scene_tick2(frame_t *frame, scene_t *scene)
{
	...
	if (sfSprite_getColor(img->sprite).a <= 10) {
		scene->event_tick = &lose_scene_tick;
...
void lose_scene_tick(frame_t *frame, scene_t *scene)
{
	...
	if (sfSprite_getColor(img->sprite).a >= 240) {
		scene->event_tick = &lose_scene_tick2;
		return;
	}
	if (color.a < 255) {
		color.a += 10;
...
{
	scene_t *intro = create_scene(LOSE);
	sfVector2i size = {.x = -1, .y = -1};
	sprite_info_t *sprite = create_sprite(0, size, LOSE_ASSET);
	sfColor color = sfSprite_getColor(sprite->sprite);

	intro->background_music = sfMusic_createFromFile(MAIN_MUSIC);
	sprite_set_scale(sprite, 4, 4);
	intro->event_tick = &lose_scene_tick;
	color.a = 0;
	sprite_set_pos(sprite, 585, 150);
